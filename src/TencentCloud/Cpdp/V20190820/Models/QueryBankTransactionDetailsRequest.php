 ...
 * @method string getQueryFlag() 获取STRING(4)，查询标志（1: 全部; 2: 转出; 3: 转入 ）
 * @method void setQueryFlag(string $QueryFlag) 设置STRING(4)，查询标志（1: 全部; 2: 转出; 3: 转入 ）
 ...
 * @method string getStartDate() 获取STRING(8)，开始日期（若是历史查询，则必输，当日查询时，不起作用。格式：20190101）
 * @method void setStartDate(string $StartDate) 设置STRING(8)，开始日期（若是历史查询，则必输，当日查询时，不起作用。格式：20190101）
 * @method string getEndDate() 获取STRING(8)，终止日期（若是历史查询，则必输，当日查询时，不起作用。格式：20190101）
...
class QueryBankTransactionDetailsRequest extends AbstractModel
{
    ...
    /**
     * @var string STRING(4)，查询标志（1: 全部; 2: 转出; 3: 转入 ）
     */
    public $QueryFlag;
    ...
    /**
     * @var string STRING(8)，终止日期（若是历史查询，则必输，当日查询时，不起作用。格式：20190101）
     */
    public $EndDate;
     ...
     * @param string $QueryFlag STRING(4)，查询标志（1: 全部; 2: 转出; 3: 转入 ）
     * @param string $PageNum STRING(10)，页码（起始值为1，每次最多返回20条记录，第二页返回的记录数为第21至40条记录，第三页为41至60条记录，顺序均按照建立时间的先后）
     * @param string $StartDate STRING(8)，开始日期（若是历史查询，则必输，当日查询时，不起作用。格式：20190101）
     * @param string $EndDate STRING(8)，终止日期（若是历史查询，则必输，当日查询时，不起作用。格式：20190101）
