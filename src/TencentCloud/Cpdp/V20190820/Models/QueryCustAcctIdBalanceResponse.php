...
注意：此字段可能返回 null，表示取不到有效值。
 * @method array getAcctArray() 获取账户信息数组
注意：此字段可能返回 null，表示取不到有效值。
 * @method void setAcctArray(array $AcctArray) 设置账户信息数组
注意：此字段可能返回 null，表示取不到有效值。
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("AcctArray",$param) and $param["AcctArray"] !== null) {
            $this->AcctArray = [];
            foreach ($param["AcctArray"] as $key => $value){
                $obj = new Acct();
                $obj->deserialize($value);
                array_push($this->AcctArray, $obj);
            }
        }
