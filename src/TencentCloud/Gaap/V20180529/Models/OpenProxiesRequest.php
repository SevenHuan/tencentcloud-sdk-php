 ...
 * OpenProxies请求参数结构体
 *
 * @method array getInstanceIds() 获取（旧参数，请切换到ProxyIds）通道的实例ID列表。
 * @method void setInstanceIds(array $InstanceIds) 设置（旧参数，请切换到ProxyIds）通道的实例ID列表。
...
更多详细信息请参阅：如何保证幂等性。
 * @method array getProxyIds() 获取（新参数）通道的实例ID列表。
 * @method void setProxyIds(array $ProxyIds) 设置（新参数）通道的实例ID列表。
 */
class OpenProxiesRequest extends AbstractModel
{
    /**
     * @var array （旧参数，请切换到ProxyIds）通道的实例ID列表。
     */
    public $InstanceIds;
...
更多详细信息请参阅：如何保证幂等性。
    ...
    /**
     * @var array （新参数）通道的实例ID列表。
     */
    public $ProxyIds;

    /**
     * @param array $InstanceIds （旧参数，请切换到ProxyIds）通道的实例ID列表。
     * @param string $ClientToken 用于保证请求幂等性的字符串。该字符串由客户生成，需保证不同请求之间唯一，最大值不超过64个ASCII字符。若不指定该参数，则无法保证请求的幂等性。
更多详细信息请参阅：如何保证幂等性。
     * @param array $ProxyIds （新参数）通道的实例ID列表。
     */
